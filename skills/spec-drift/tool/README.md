...
| `verdicts <file>… [--book <name>]…` | — | grades the rows under the `inventory` header row of each file (the totals table, prose and fenced code are skipped): a row without exactly six cells; a row with no verdict or none of the four; a ① that names no existing entry (matched as a whole ID) or quotes wording that is not in the ledger verbatim (quotes are straight or curly double quotes or CJK corner brackets, read outside backtick code spans, in the verdict and reason cells); a ② that names neither an existing entry ID nor a book given with `--book`; a ③ whose reason matches the phrase "the ledger does not say so" (a phrase screen, not a reading of the reason's logic); a ②, ③ or ④ with no reason; a file with no verdict table. Whether a genuine quote covers every decision the line makes stays a judgement | 0 when every row passes; 1 when any fails; 2, on stderr, when a file cannot be read |
