...
| `uncovered` | — | reports the symbols in the hot-zone files (derived from the ledger anchors, optionally unioned with the `hot_zone` field of `.spec-drift.json`; any `tests` directory excluded) that no entry anchors | usually 0; 2 when a `hot_zone` item exists under several `code_root`s (its owner cannot be determined — a configuration error, see below) |
