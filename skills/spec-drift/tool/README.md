...
| `uncovered` | — | reports the symbols in the hot-zone files (derived from the ledger anchors, optionally unioned with the `hot_zone` field of `.spec-drift.json`; any `tests` directory excluded, and `node_modules` excluded from the configured part) that no entry anchors; a `hot_zone` item that adds no file once `tests` and `node_modules` are excluded is reported with a `⚠` line | usually 0; 2 when a `hot_zone` item exists under several `code_root`s (its owner cannot be determined — a configuration error, see below) |
...
| `verdicts <file>… [--book <name>]…` | — | grades the rows under the `inventory` header row of each file (the totals table, prose and fenced code, backtick or tilde, are skipped; the header must be followed by a six-column separator): a row without exactly six cells; a row with no verdict or none of the four; a ① that names no existing entry (matched as a whole ID) or quotes wording that is not in the ledger verbatim once runs of whitespace are collapsed (quotes are straight or curly double quotes or CJK corner brackets, read outside code spans, cell by cell); a ② that names neither an existing entry ID nor a book given with `--book`; a ③ whose reason matches the phrase "the ledger does not say so" (a phrase screen, not a reading of the reason's logic); a ②, ③ or ④ with no reason; a file with no verdict table. Whether a genuine quote covers every decision the line makes stays a judgement | 0 when every row passes; 1 when any fails; 2, on stderr, when a file cannot be read |
